...
export function UserPropertyListScreen({ kind, title, emptyTitle, emptyText }: Props) {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      setProperties((data as Record<string, Property[]>)[kind]);
    } catch (requestError) {
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: colors.white, borderRadius: 22, borderWidth: 1, borderColor: colors.line, overflow: 'hidden' }, image: { width: '100%', height: 190, backgroundColor: '#EAE2DE' }, cardBody: { padding: 16 }, badge: { alignSelf: 'flex-start', color: colors.primary, backgroundColor: colors.primarySoft, borderRadius: 100, paddingHorizontal: 9, paddingVertical: 4, fontSize: 9, lineHeight: 12, fontWeight: '900', letterSpacing: 1 }, title: { color: colors.ink, fontSize: 18, lineHeight: 24, fontWeight: '800', marginTop: 9 }, location: { color: colors.muted, fontSize: 12, lineHeight: 18, marginTop: 5 }, footer: { borderTopWidth: 1, borderTopColor: colors.line, marginTop: 14, paddingTop: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, price: { color: colors.primary, fontSize: 17, lineHeight: 23, fontWeight: '900' }, meta: { flex: 1, color: colors.muted, fontSize: 10, lineHeight: 15, textAlign: 'right' },
});
