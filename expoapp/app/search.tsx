  ...
  return (
    <ThemedView style={styles.screen}>
      <SafeAreaView edges={['top']} style={styles.safeArea}>
        <Header style={styles.topBar}><Pressable accessibilityLabel="Go back" style={styles.backButton} onPress={() => router.back()}><Chevron direction="left" size={28} color={colors.ink} strokeWidth={2.5} /></Pressable><Text style={styles.topTitle}>Search properties</Text><View style={styles.headerSpacer} /></Header>
        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadProperties(true)} tintColor={colors.primary} colors={[colors.primary]} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.searchBox}><Text style={styles.searchIcon}>⌕</Text><TextInput autoFocus={!params.q} value={query} onChangeText={setQuery} onSubmitEditing={submitSearch} returnKeyType="search" placeholder="Area, property or landmark" placeholderTextColor="#9B908D" style={styles.searchInput} /><Pressable style={styles.searchButton} onPress={submitSearch}><Text style={styles.searchButtonText}>Search</Text></Pressable></View>

          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filters}>
            {(['all', 'sale', 'rent'] as Purpose[]).map((item) => <Pressable key={item} onPress={() => setPurpose(item)} style={[styles.filter, purpose === item && styles.filterActive]}><Text style={[styles.filterText, purpose === item && styles.filterTextActive]}>{item === 'all' ? 'All listings' : `For ${item}`}</Text></Pressable>)}
          </ScrollView>
          {availableTypes.length > 0 && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.typeFilters}><Pressable onPress={() => setSelectedType('all')} style={[styles.typeFilter, selectedType === 'all' && styles.typeFilterActive]}><Text style={[styles.typeText, selectedType === 'all' && styles.typeTextActive]}>Any type</Text></Pressable>{availableTypes.map((type) => <Pressable key={type} onPress={() => setSelectedType(type)} style={[styles.typeFilter, selectedType === type && styles.typeFilterActive]}><Text style={[styles.typeText, selectedType === type && styles.typeTextActive]}>{type}</Text></Pressable>)}</ScrollView>}

          <View style={styles.resultHeader}><View><Text style={styles.resultTitle}>{submittedQuery ? `Results for “${submittedQuery}”` : 'Properties in Nepal'}</Text><Text style={styles.resultCount}>{loading ? 'Searching…' : `${results.length} ${results.length === 1 ? 'property' : 'properties'} found`}</Text></View></View>

          {loading && <View style={styles.state}><ActivityIndicator size="large" color={colors.primary} /><Text style={styles.stateText}>Finding properties…</Text></View>}
          {!loading && error && <View style={styles.state}><Text style={styles.stateTitle}>Search unavailable</Text><Text style={styles.stateText}>{error}</Text><Pressable style={styles.retryButton} onPress={() => void loadProperties()}><Text style={styles.retryText}>Try again</Text></Pressable></View>}
          {!loading && !error && results.length === 0 && <View style={styles.state}><View style={styles.emptyIcon}><Text style={styles.emptyIconText}>⌕</Text></View><Text style={styles.stateTitle}>No matching properties</Text><Text style={styles.stateText}>Try another area, property type, or remove a filter.</Text><Pressable style={styles.clearButton} onPress={() => { setQuery(''); setSubmittedQuery(''); setPurpose('all'); setSelectedType('all'); }}><Text style={styles.clearText}>Clear filters</Text></Pressable></View>}

          <View style={styles.results}>{!loading && !error && results.map((property) => <Pressable key={property.id} style={styles.card} onPress={() => router.push({ pathname: '/property/[id]', params: { id: String(property.id) } })}><Image source={{ uri: imageUrl(property.images?.[0] || property.mainMedia) }} style={styles.cardImage} contentFit="cover" transition={200} /><View style={styles.cardBody}><View style={styles.cardTop}><Text style={styles.cardType}>{(property.property_types?.[0] || property.types?.[0]?.name || 'Property').toUpperCase()}</Text><Text style={styles.heart}>♡</Text></View><Text style={styles.price}>{priceLabel(property)}</Text><Text numberOfLines={2} style={styles.cardTitle}>{property.title || 'Property listing'}</Text><Text numberOfLines={1} style={styles.location}>⌖  {locationLabel(property)}</Text><View style={styles.cardFooter}><Text numberOfLines={1} style={styles.specs}>{property.specs || 'View details'}</Text><Text style={styles.arrow}>›</Text></View></View></Pressable>)}</View>
        </ScrollView>
      </SafeAreaView>
    </ThemedView>
  );
...
const styles = StyleSheet.create({
  ...
  searchBox: { minHeight: 58, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: 18, paddingLeft: 14, paddingRight: 6, flexDirection: 'row', alignItems: 'center' }, searchIcon: { color: colors.primary, fontSize: 24, lineHeight: 28, marginRight: 7, transform: [{ rotate: '-15deg' }] }, searchInput: { flex: 1, height: 52, minWidth: 0, color: colors.ink, fontSize: 13, fontFamily: 'GoogleSansRegular' }, searchButton: { height: 44, paddingHorizontal: 16, borderRadius: 13, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center' }, searchButtonText: { color: colors.white, fontSize: 12, lineHeight: 17, fontWeight: '800' },
