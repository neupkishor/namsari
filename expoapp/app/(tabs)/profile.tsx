    ...
    <ThemedView style={styles.screen}>
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.sectionTitle}>Tools</Text><View style={styles.toolsGrid}><ToolCard icon="▣" title="EMI Calculator" subtitle="Estimate loan payments" href="/emi-calculator" /><ToolCard icon="◷" title="Date Converter" subtitle="Convert dates easily" href="/date-converter" /><ToolCard icon="↔" title="Unit Converter" subtitle="Convert property areas" href="/unit-converter" /></View>
...
const styles = StyleSheet.create({
  ...
  helpCard: { backgroundColor: '#F0E3C3', borderRadius: 22, marginTop: 22, padding: 20 }, helpKicker: { color: '#735A18', fontSize: 9, lineHeight: 13, fontWeight: '900', letterSpacing: 1.2 }, helpTitle: { color: colors.ink, fontSize: 17, lineHeight: 23, fontWeight: '800', marginTop: 5 }, helpText: { color: '#695F45', fontSize: 11, lineHeight: 17, marginTop: 5 }, helpButton: { alignSelf: 'flex-start', backgroundColor: colors.ink, borderRadius: 100, paddingHorizontal: 15, paddingVertical: 9, marginTop: 14 }, helpButtonText: { color: colors.white, fontSize: 10, lineHeight: 15, fontWeight: '800' }, toolsGrid: { backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: 20, paddingHorizontal: 14 }, toolCard: { minHeight: 70, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.line }, toolCardLast: { borderBottomWidth: 0 }, toolIcon: { width: 40, height: 40, borderRadius: 13, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center' }, toolIconText: { color: colors.primary, fontSize: 19, fontWeight: '700' }, toolCopy: { flex: 1, marginLeft: 12 }, toolTitle: { color: colors.ink, fontSize: 13, lineHeight: 18, fontWeight: '700' }, toolSubtitle: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 2 }, toolArrow: { color: colors.primary, fontSize: 25, lineHeight: 29 },
});

function ToolCard({ icon, title, subtitle, href }: { icon: string; title: string; subtitle: string; href: '/emi-calculator' | '/date-converter' | '/unit-converter' }) { return <Pressable style={styles.toolCard} onPress={() => router.push(href)}><View style={styles.toolIcon}><Text style={styles.toolIconText}>{icon}</Text></View><View style={styles.toolCopy}><Text style={styles.toolTitle}>{title}</Text><Text style={styles.toolSubtitle}>{subtitle}</Text></View><Text style={styles.toolArrow}>›</Text></Pressable>; }
